...
const SettingInfo readerSettings[readerSettingsCount] = {
    ...
    SettingInfo::Toggle("Standardize Formatting", &CrossPointSettings::standardizeFormatting)};
...
void SettingsActivity::enterCategory(int categoryIndex) {
  ...
  int settingsCount = 0;
  bool hasSleepBmps = checkSleepBmps();

  switch (categoryIndex) {
    case 0:  // Display
      settingsList = displaySettings;
      // Adjust count if sleep BMPs don't exist (skip "Select Sleep BMP" at index 2)
      settingsCount = hasSleepBmps ? displaySettingsCount : displaySettingsCount - 1;
      break;
    case 1:  // Reader
  ...
  // For Display category, filter out "Select Sleep BMP" if no BMPs exist
  if (categoryIndex == 0 && !hasSleepBmps) {
    // Create a filtered list without "Select Sleep BMP"
    static SettingInfo filteredDisplaySettings[displaySettingsCount - 1];
    int filteredIndex = 0;
    for (int i = 0; i < displaySettingsCount; i++) {
      if (i != 2) {  // Skip index 2 which is "Select Sleep BMP"
        filteredDisplaySettings[filteredIndex++] = displaySettings[i];
      }
    }
    enterNewActivity(new CategorySettingsActivity(renderer, mappedInput, categoryNames[categoryIndex],
                                                  filteredDisplaySettings, settingsCount, [this] {
                                                    exitActivity();
                                                    updateRequired = true;
                                                  }));
  } else {
